...
class Bitmap
  ...
  #   starting_position: posizione iniziale dove cominciare a scrivere nel paragrafo
  # @param [Integer] x
  # @param [Integer] y
  # @param [Integer] width
  # @param [String] text
  # @param [Integer] max
  # @param [Boolean] test
  # @param [Hash, nil] starting_position
  ...
  def draw_text_paragraph(x, y, width, paragraph, mh, test = false)
    ...
    {:x => xy[:x], :y => xy[:y] - y + line_height, :lines => @paragraph_lines, :chars => 0}
  end
...
module Emoji
  ...
  # @param [String] emoji_char
  def self.has?(emoji_char)
    all_chars.include?(emoji_char)
  end

  # restituisce il carattere emoji selezionato
  # @param [String] emoji_char
  def self.get(emoji_char)
    LIST[emoji_char]
  end
  ...
  def self.elements
    ...
    @elements = []
    LIST.each_pair { |char, icon|
      type = find_pair(char)
      @elements.push(Emojie.new(char, type, icon))
    }
    return @elements
  end

  def self.find_pair(char)
    EMOJIS.each_pair do |type, list|
      return type if list.include?(char)
    end
    :other
  end
end
...
# ** Tone
#------------------------------------------------------------------------------
#  Possibilità di applicare un tono partendo dal colore
#==============================================================================
class Tone
