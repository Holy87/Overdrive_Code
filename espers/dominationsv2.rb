...
class Scene_Dominations < Scene_MenuBase
  ...
  def start
    ...
    create_affinity_window
    #create_powerups_window
    #create_powerup_effect_window
    all_windows_invisible
  end

  def update_dominations_parameter
    $game_party.unlocked_dominations.each{|domination| domination.setup_esper_param_bonus}
  end
  ...
  def create_affinity_window
    rec = subwindows_position
    @affinity_window = Window_DominationAffinity.new(rec.x, rec.y, rec.width, rec.height)
    @detail_window.bind(@affinity_window)
  end
  ...
  def back_to_command
    ...
    @power_window.index = -1 unless @power_window.nil?
  end
  ...
  def all_windows_invisible
    ...
    @affinity_window.visible = false
    @power_window.visible = false unless @power_window.nil?
...
class Window_DominationInfo < Window_Base
  ...
  def draw_icon_param(actor, x, y, i)
    ...
    draw_text(x + 24, y, 100, line_height, param[0])
  end
  ...
  def draw_actor_hpmp(actor, x, y)
    ...
    draw_text(x + ts, y, 100, line_height, actor.maxhp)
    draw_text(x + ts, y + line_height, 100, line_height, actor.maxmp)
  end
  ...
  def draw_proper_user(actor, x, y)
    ...
    draw_text(x, y, 100, line_height, Vocab.in_use_by)
...
class Window_DominationList < Window_Selectable
  ...
  def set_domination_window(window)
    @domination_window = window
  end
...
class Window_DominationSkills < Window_Selectable
  ...
  def skill_enabled?(skill)
    !domination.skill_hidden?(skill.id)
  end
...
class Window_DominationEmpowerments < Window_Selectable
  ...
  def draw_item_cost(item, rect, enabled)
    ...
    change_color(crisis_color, enabled)
    text = sprintf('%d %s', cost, Vocab.ap)
    draw_text(rect, text, 2)
  end
...
class Window_DominationAP < Window_Base
  # inizializzazione
  def initialize(x, y, width)
    super(x, y, width, fitting_height(1))
    self.openness = 0
  ...
  def refresh
    contents.clear
    return if @domination.nil?
    domination.ap > 0 ? draw_normal_ap : draw_no_master
  end
  ...
  def draw_normal_ap
    ...
    text = sprintf(Vocab::esper_ap_help, domination.esper_master.name)
    draw_text(32, 0, contents_width - 32, line_height, text)
    change_color(crisis_color)
    draw_text(0, 0, contents_width, line_height, domination.ap, 2)
  end
...
class Window_DominationAffinity < Window_Base
  def refresh
    contents.clear
    return if domination.nil?
    draw_characters_affinity
  end

  def draw_characters_affinity
    affinities = EsperConfig.get_actor_affinities(domination.id)
    affinities.each_with_index { |a, i| draw_actor_affinity(a, i) }
  end

  # @param [Esper_Affinity] actor_affinity
  # @param [Integer] index
  def draw_actor_affinity(actor_affinity, index)
    width = contents_width / 2
    x = index % 2 * width
    y = index / 2 * line_height
    draw_actor_little_face(actor_affinity.actor, x, y)
    draw_affinity(x + 24, y, actor_affinity.affinity)
  end

  def draw_affinity(x, y, value)
    value.times { |i| draw_icon(EsperConfig::AFFINITY_ICON, x + (i * 24), y) }
  end
