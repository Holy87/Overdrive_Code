...
#===============================================================================
class Window_BoardWriter2 < Window_Base
  ...
  def draw_remaning_characters
    ...
    draw_text(0, contents.height - 12, contents.width - 10, 12, remaining_characters, 2)
    contents.font.size = old_size
  end

  # Il numero di caratteri restanti
  def remaining_characters
    rem = @max_characters - (@text.size + contents.emojis)
    [rem, 0].max
  end
  ...
  def writable?
    open? && self.visible && self.active && emoji_window_closed?
  end
  ...
  def update_typing
    if Input.typing?
      if remaining_characters > 0 #@text.size + contents.emojis*5 < @max_char
        @text += Input.key_type
      else
        Sound.play_buzzer
      end
  ...
  def update_character_typing
    char = Input.any_keyboard_input?
