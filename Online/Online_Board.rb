...
module Vocab
  ...
  def self.writing_help
    'Invio per spedire, F5 per inserire una emoji.'
  end
...
class BoardMessage
  ...
  def initialize(data)
    @name = data['player_name'] || data['old_name']
    @avatar = data['player_face']
    @level = data['player_level']
    @date = get_date(data['date'])
    @banned = data['banned']
    @message = data['message']
    @registered = !data['player_name'].nil?
...
class Scene_MessageBoard < Scene_MenuBase
  ...
  def params
    base64_encode($game_temp.sphere_id);
  end
  ...
  def downloaded(response)
    ...
    case data
    when '-1';
      @info_window.set_help(:data_error) #show_dialog(Vocab.data_error)
    when '0';
      @info_window.set_help(:downloaded) #show_dialog(Vocab.no_messages)
    when /[.]*<br>[.]*/i
      @info_window.set_help(:downloaded)
      elaborate_data(data)
    else
      ; @info_window.set_help(:connection_error) #show_dialog(Vocab.connection_error)
    end
  end
...
class Window_BoardMessage < Window_Selectable
  ...
  def draw_item(index)
    ...
    cl = Color_Rule.new(/\[/, /\]/, crisis_color, normal_color)
...
class Window_BoardChoice < Window_Command
  ...
  def window_height
    Graphics.width - fitting_height(2);
  end
