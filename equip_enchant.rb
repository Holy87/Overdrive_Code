...
module EquipEnchant
  ...
  #-----------------------------------------------------------------------------
  def special_text; @special_text; end
  #--------------------------------------------------------------------------
  ...
  def add_weapon_effect(effect)
    ...
    case effect[0]
      when :mhp; @stat_per[:hp] += value
      when :mmp; @stat_per[:mp] += value
      when :atk; @stat_per[:atk] += value
      when :def; @stat_per[:def] += value
      when :spi; @stat_per[:spi] += value
      when :agi; @stat_per[:agi] += value
      when :cri; @cri += value
      when :hit; @hit += value
      when :eva; @eva += value
      when :odd; @odds += value
      when :ele; @element_set.push(value)
      when :sta; @state_set.push(value)
      when :stm; @minus_state_set.push(value)
      when :esc; @escape_bonus += value
      when :ela; @element_amplify[id] = value
      when :hea; @heal_amplify = value
      when :rec; @recharge_value = value
      when :chb; @charge_bonus = value
      when :atb; @atb_base = value
      when :vmp; @vampire_rate += value
      when :mpr; @mp_rate += value/100.0
      when :its; @item_save += value/100.0
      when :anm; @animation_id = value
      when :mst; @magic_states_plus.push(value)
      when :hst; @heal_states_plus.push(value)
      when :inc; @incentive += value
      when :snd; @sin_durab += value
      when :snb; @sin_bonus += value/100.0
      when :dmb; @dom_bonus += value/100.0
      when :stl; @__steal_prob_plus += value
      when :ico; @icon_index = value
      when :lhm; setmhplow(effect)
      when :lhp; setmhplowp(effect)
      when :ira; @anger_bonus += value
      when :iri; @anger_init += value
  ...
  #--------------------------------------------------------------------------
  def enchant; @enchant_state += 1; end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  def customized?; enchant_state > 0 || @power_up > 0; end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  def enchant_state; @enchant_state || 0; end
  #--------------------------------------------------------------------------
...
class RPG::Item
  ...
  #--------------------------------------------------------------------------
  def enchantable?; false; end
  #--------------------------------------------------------------------------
  # * E neanche modificato
  #--------------------------------------------------------------------------
  def customized?; false; end
  #--------------------------------------------------------------------------
  # * E neanche potenziato
  #--------------------------------------------------------------------------
  def power_up_possible?; false; end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  def enchant_scroll?; @enchant_tier > 0; end
  #--------------------------------------------------------------------------
  # * non può essere potenziato un item...
  #--------------------------------------------------------------------------
  def powered?;false;end
end
...
class RPG::Weapon
  ...
  #--------------------------------------------------------------------------
  def mod(symbol); H87Enchant.get_w_mod(symbol); end
  #--------------------------------------------------------------------------
  # * Restituisce l'attacco
  #--------------------------------------------------------------------------
  def atk; @atk + (@atk.abs * mod(:atk) * enchant_state).to_i; end
  #--------------------------------------------------------------------------
  # * Restituisce la difesa
  #--------------------------------------------------------------------------
  def spi; @spi + (@spi.abs * mod(:spi) * enchant_state).to_i; end
  #--------------------------------------------------------------------------
  # * Restituisce lo spirito
  #--------------------------------------------------------------------------
  def def; @def + (@def.abs * mod(:def) * enchant_state).to_i; end
  #--------------------------------------------------------------------------
  # * Restituisce l'agilità
  #--------------------------------------------------------------------------
  def agi; @agi + (@agi.abs * mod(:agi) * enchant_state).to_i; end
  #--------------------------------------------------------------------------
...
class RPG::Armor
  ...
  # * Restituisce il modificatore dell'attributo
  #--------------------------------------------------------------------------
  def mod(symbol); H87Enchant.get_a_mod(symbol); end
  #--------------------------------------------------------------------------
  # * Restituisce l'attacco
  #--------------------------------------------------------------------------
  def atk; @atk + (@atk.abs * mod(:atk) * enchant_state).to_i; end
  ...
  # * Restituisce il prezzo
  #--------------------------------------------------------------------------
  def price; @price + (@price * mod(:price) * enchant_state).to_i; end
  #--------------------------------------------------------------------------
  # * Restituisce false
  #--------------------------------------------------------------------------
  def enchantable?; false; end
...
class Window_EquipEnchantList < Window_Selectable
  ...
  #--------------------------------------------------------------------------
  def enable?(item); item and item_number(item) > 0; end
  #--------------------------------------------------------------------------
...
class Window_EnchantState < Window_Base
  ...
  def status_text
    case @state
      when 0; H87Enchant::ST1
      when 1; H87Enchant::ST2
      when 2; H87Enchant::STQ
      when 3; H87Enchant::ST3
      when 4; H87Enchant::ST4
      when 5; H87Enchant::ST5
      else;   'ERRORE 01'
    end
  end
  #--------------------------------------------------------------------------
...
class GraphicUnifier
  ...
  # @attr [RPG::Item] scroll
  # @attr [RPG::Weapon] equip
  # @attr [RPG::Weapon] product
  #--------------------------------------------------------------------------
  attr_reader :x
  attr_reader :y
  attr_reader :scroll
  ...
  #--------------------------------------------------------------------------
  def cent_x; self.x + (@width/2); end
  #--------------------------------------------------------------------------
  # * Restituisce la coordinata Y del centro
  #--------------------------------------------------------------------------
  def cent_y; @spr_y + (@spr_h/2); end
  #--------------------------------------------------------------------------
  # * Aggiornamento
  #--------------------------------------------------------------------------
  def update
